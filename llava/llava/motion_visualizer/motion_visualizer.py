...
def main(motion_path, i):
    with open(motion_path, "rb") as f:
        motion_list = pickle.load(f)  # motion data is a dictionary
    # random select one id from motion_list
    motion_id = list(motion_list.keys())[i]
    ...
    out_motion = motion_data_transform_input['segment_tXYZ']
    out_name = os.path.split(motion_path)[1].split('.')[0]

    # load the json list and get cooresponding text ground truth
    json_path = '/scratch/inf0/user/jianwang/nymeria/ego4o_input_json.jsonl'
    with open(json_path, 'r') as f:
        lines = f.readlines()
    list_data_dict = [json.loads(line) for line in lines]
    text = [data_dict['conversations'] for data_dict in list_data_dict if data_dict['id'] == motion_id]
    print(text)

    out_path = os.path.join(os.path.split(motion_path)[0], f'{out_name}_{i}.gif')
    plot_3d_motion(out_path, out_motion,
                   None, 'motion', (10, 10), 20, 2, skeleton, None)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--motion_index", type=int)
    args = parser.parse_args()
    main(args.motion_path, args.motion_index)
