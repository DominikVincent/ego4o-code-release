...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ['mm_projector', 'vision_tower', 'vision_resampler',
                           'imu_tokenizer_postprocess', 'vq_net_decoder_preprocess', 'vq_net_postprocess']
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        if not training_args.lora_enable:
            ...
            print(f"Loading pretrained vq vae from {model.get_model().config.pretrained_vqvae_path}", flush=True)
            pretrain_vae_state_dict = torch.load(model.get_model().config.pretrained_vqvae_path)
            if 'state_dict' in pretrain_vae_state_dict:
                pretrain_vae_state_dict = pretrain_vae_state_dict['state_dict']
            model.get_model().vq_net.load_state_dict(pretrain_vae_state_dict, strict=True)
        ...
        if not training_args.lora_enable:
            ...
            if 'state_dict' in pretrain_imu_tokenizer_state_dict:
                pretrain_imu_tokenizer_state_dict = pretrain_imu_tokenizer_state_dict['state_dict']
            pretrain_imu_tokenizer_state_dict = {k.replace("transformer_traj_model.", ""): v for k, v in
                                        pretrain_imu_tokenizer_state_dict.items() if k.startswith("transformer_traj_model.")}
        ...
        # print('load the clip in the model', flush=True)
        # model.get_model().imu_tokenizer.load_and_freeze_clip()
