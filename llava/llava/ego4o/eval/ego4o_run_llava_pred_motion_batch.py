...
def eval_model(args):
    ...
    motion_list = load_motion(motion_path)

    for out_index in tqdm(range(len(motion_list))):
        motion_data = motion_list[out_index]
        motion_id = motion_data['motion_id']
        motion_file = motion_data['motion_file']
        pred_coords = motion_data['pred_coords']
        # get the image file
        image_base_dir = '/scratch/inf0/user/jianwang/nymeria/images'
        # split the first _ from motion id, get the first part and second part
        image_id, seq_name = motion_id.split('_', 1)
        image_id = f"{int(image_id):06d}"
        image_path = os.path.join(image_base_dir, seq_name, image_id, 'frame_3.jpg')
        if not os.path.exists(image_path):
            image_path = os.path.join(image_base_dir, seq_name, image_id, 'frame_1.jpg')

        print(f"image_path: {image_path}")

        print(f"motion_id: {motion_id}")
        motion_data_transform_input = {
            'segment_tXYZ': pred_coords,
        }
    ...
    with open(os.path.join(save_path, 'output_text_list.json'), 'w') as f:
        json.dump(output_text_list, f)
