...
class Ego4oMetaModel:
    ...
    def initialize_ego4o_modules(self, config):
        ...
        if self.load_motion_encoder:
            ...
            self.vq_net.load_state_dict(torch.load(config.pretrained_vqvae_path), strict=True)
        ...
        # self.imu_tokenizer = TransformerAutoencoder_withCodes_hml_G2_noTraj(input_dim=3 + 6)
        # imu_tokenizer_state_dict = torch.load(config.pretrained_imu_tokenizer_path)['transformer_traj_model']
...
class Ego4oConfig(LlamaConfig):
    ...
    pretrained_imu_tokenizer_path = '/CT/EgoMocap/work/EgoOmniMocap/work_dirs/train_imuposer_dataset_text_imumask/best_C-MPJPE_epoch_3.pth'
...
class Ego4oForCausalLM(LlavaLlamaForCausalLM):
    ...
    def prepare_inputs_labels_for_motion(
        ...
        images, image_sizes=None, motion=None):

        # first use the parent function to get the inputs and labels
        vision_tower = self.get_vision_tower()
        if vision_tower is None or (images is None and motion is None) or input_ids.shape[1] == 1:
        ...
        new_input_embeds = []
        new_labels = []
        # convert from image idx to motion index
        # cur_image_idx = 0
        cur_motion_idx = 0
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if num_motions == 0:
                ...
                cur_motion_idx += 1
                continue  # work on next sentence in the batch

            # pdb.set_trace()

            motion_token_indices = [-1] + torch.where(cur_input_ids == MOTION_TOKEN_INDEX)[0].tolist() + [cur_input_ids.shape[0]]
            # motion_token_indices = [-1] + torch.where(cur_input_ids == MOTION_TOKEN_INDEX).tolist() + [cur_input_ids.shape[0]]
            cur_input_ids_no_motion = []
            cur_labels = labels[batch_idx]
            cur_labels_no_motion = []
            for i in range(len(motion_token_indices) - 1):
                cur_input_ids_no_motion.append(cur_input_ids[motion_token_indices[i] + 1: motion_token_indices[i + 1]])
                cur_labels_no_motion.append(cur_labels[motion_token_indices[i] + 1:motion_token_indices[i + 1]])
            split_sizes = [x.shape[0] for x in cur_labels_no_motion]
            cur_input_embeds = self.get_model().embed_tokens(torch.cat(cur_input_ids_no_motion))
            cur_input_embeds_no_motion = torch.split(cur_input_embeds, split_sizes, dim=0)
            cur_new_input_embeds = []
            cur_new_labels = []

            assert num_motions == 1 or num_motions == 2, f"Error: the number of motion tokens is not 1 or 2, {num_motions}"

            for i in range(num_motions + 1):
                cur_new_input_embeds.append(cur_input_embeds_no_motion[i])
                cur_new_labels.append(cur_labels_no_motion[i])
                if i < num_motions:
                    cur_motion_features = motion_features[cur_motion_idx]
                    cur_motion_idx += 1
                    cur_new_input_embeds.append(cur_motion_features)
                    if len(cur_motion_features) % 49 != 0:
                        print(f"Error: the length of motion hidden state is not a multiple of 49, ")
                        # pdb.set_trace()
                    cur_new_labels.append(torch.full((cur_motion_features.shape[0],), IGNORE_INDEX,
                                                     device=cur_labels.device, dtype=cur_labels.dtype))
                    # cur_new_labels.append(torch.full((cur_motion_features.shape[0],), self.motion_token_index,
                    #                                  device=cur_labels.device, dtype=cur_labels.dtype))
    ...
    def forward(
        ...
        if labels is not None and self.is_pretrain is False:
            # note: dangerous operation!!!!
            # flat the motion_hml_source
            motion_hml_source = [item for sublist in motion_hml_source for item in sublist]
            motion_token_locations = []

            for batch_idx in range(labels.shape[0]):
                input_tensor = labels[batch_idx]
                # check if all the input_tensor is IGNORE_INDEX
                if torch.all(input_tensor == IGNORE_INDEX):
                    print(f"Warning: all the input tensor is IGNORE_INDEX, ", flush=True)
                    print("Label: ", labels[batch_idx], flush=True)
                    print('Input ID: ', input_ids_clone[batch_idx], flush=True)
                    motion_token_locations.append([])
                    continue
                # Find the first non IGNORE_INDEX index from the beginning
                first_non_negative_index = (input_tensor != IGNORE_INDEX).nonzero(as_tuple=True)[0].min().item()

                # Find the last non IGNORE_INDEX index from the end
                last_non_negative_index = (input_tensor != IGNORE_INDEX).nonzero(as_tuple=True)[0].max().item()

                # Slice the tensor to omit IGNORE_INDEX's from the beginning and end
                trimmed_tensor = input_tensor[first_non_negative_index:last_non_negative_index + 1]

                # Find all locations of IGNORE_INDEX in the trimmed tensor
                negative_ones_indices = (trimmed_tensor == IGNORE_INDEX).nonzero(as_tuple=True)[0].tolist()

                # Adjust indices to reflect positions in the original list
                adjusted_indices = [index + first_non_negative_index for index in negative_ones_indices]

                motion_token_locations.append(adjusted_indices)

        if labels is not None and self.is_pretrain is False:
            # get the hidden states from the model

            # calculate the loss between the output of the vq vae and the input motion
            hidden_state_list = result.hidden_states
            hidden_state_last = hidden_state_list[-1]  # [batch_size, seq_len, emb_dim]
            # hidden_state_last = self.model.vq_net_decoder_preprocess(hidden_state_list[-1])  # [batch_size, seq_len, emb_dim]
            # select the motion part

            motion_hidden_state_list = []

            for batch_idx in range(labels.shape[0]):
                # get the indices of the motion tokens
                # motion_token_indices = torch.where(labels[batch_idx] == self.motion_token_index)[0]
                motion_token_indices = motion_token_locations[batch_idx]
                if len(motion_token_indices) == 0: # no motion token in the sentence
                    continue

                # get the motion part from hidden state
                motion_hidden_state = hidden_state_last[batch_idx, motion_token_indices]
                if len(motion_hidden_state) % 49 != 0:
                    print(f"Error: the length of motion hidden state is not a multiple of 49, ", flush=True)
                    print("Label: ", labels[batch_idx], flush=True)
                    motion_hidden_state = motion_hidden_state[:-(len(motion_hidden_state) % 49)]

                motion_hidden_state = self.model.vq_net_decoder_preprocess(motion_hidden_state)
                # split the motion hidden state by motion token length: 49
                motion_hidden_state_list.extend(torch.split(motion_hidden_state, 49, dim=0))

            vq_net_decoder_input = torch.stack(motion_hidden_state_list, dim=0)

            # pdb.set_trace()

            bs, N_token, emb_dimx6 = vq_net_decoder_input.shape
            vq_net_decoder_reshape = vq_net_decoder_input.reshape(bs, N_token, 6, emb_dimx6 // 6)
            vq_net_decoder_permute = vq_net_decoder_reshape.permute(0, 2, 1, 3)  # [batch_size, 6, N_token, emb_dim]

            # put into the vq vae decoder
            # vq_net_decoder_input = vq_net_decoder_input.permute(0, 2, 1)  # [batch_size, emb_dim, seq_len]
            codes_pick_gumbel_softmax = F.gumbel_softmax(vq_net_decoder_permute, tau=1, eps=1e-10, hard=True, dim=-1)

            x_quantized_fromIds = self.get_model().vq_net.vqvae.get_x_quantized_from_x_ids(codes_pick_gumbel_softmax.permute(0, 2, 3, 1).contiguous())

            vq_net_decoder_output = self.get_model().vq_net.vqvae.forward_decoder_from_quantized_codes(x_quantized_fromIds)

            # motion_hml = motion_hml.squeeze(-2)
            # select motion hml with source = gpt
            gpt_indices = [index for index, string in enumerate(motion_hml_source) if string == 'gpt']
            motion_hml = motion_hml[gpt_indices]


            # calculate the loss between the output of the vq vae and the input motion
            loss_motion = self.get_model().config.motion_recon_weight * F.mse_loss(vq_net_decoder_output, motion_hml)

            result.loss += loss_motion

            ############## calculate the motion token loss
            log_probs = F.log_softmax(vq_net_decoder_permute, dim=-1)  # [batch_size, 6, N_token=49, emb_dim=128]
            motion_code_index_loss = motion_code_index[gpt_indices].permute(0, 2, 1)  # [batch_size, 6, N_token=49]
            motion_code_index_loss = motion_code_index_loss.detach()

            motion_token_loss = F.nll_loss(log_probs.reshape(-1, log_probs.shape[-1]), motion_code_index_loss.reshape(-1))

            result.loss += motion_token_loss


        # add motion token loss
