...
class Ego4oMetaModel:
    ...
    def encode_motion(self, motion, debug=False):
        ...
        output_motion = self.vq_net_postprocess(embedding_motion)
...
class Ego4oForCausalLM(LlavaLlamaForCausalLM):
    ...
    def prepare_inputs_labels_for_motion(
        ...
        motion_features = self.model.encode_motion(motion, debug=False)
    ...
    def forward(
        ...
        if labels is not None and self.is_pretrain is False:
            ...
            if labels is not None and self.is_pretrain is False:
                for batch_idx in range(labels.shape[0]):
                    ...
                    if torch.all(input_tensor == IGNORE_INDEX):
                        ...
                        print('Input ID: ', input_ids[batch_idx], flush=True)
            ...
            vq_net_decoder_output = self.model.vq_net.vqvae.decoder(vq_net_decoder_input)
            ...
            # calculate the loss between the output of the vq vae and the input motion
            loss_motion = F.mse_loss(vq_net_decoder_output, motion_hml)
