...
class TransformerAutoencoder_withCodes_hml_G2_noTraj(TransformerAutoencoder_hml):
    def __init__(self,
        ...
        super(TransformerAutoencoder_withCodes_hml_G2_noTraj, self).__init__()
    ...
    def forward(self, x, x_text, x_image=None):  # Toekn size: T
        ...
        input_feature = self.embed_text(enc_text).unsqueeze(0)
...
class TransformerIMUDecoder(TransformerAutoencoder_hml):
    def __init__(self,
                 input_dim=3,
                 dropout=0.1,
                 num_emb=128,
                 ):
        super(TransformerIMUDecoder, self).__init__()
        self.input_dim = input_dim
        # if self.input_dim > 3:
        #    self.input_dim *= 28
        self.codesTimeLen = 49
        ...
        self.linear_mid_codeIdx = nn.Linear(self.model_dim1, self.model_dim2)
        self.linear_text_condeIdx = nn.Linear(self.model_dim1, self.model_dim2)
        # self.linear_mid_traj = nn.Linear(self.model_dim1, self.model_dim2)
        ...
        # Define transformer decoder
        self.transformer_codeIdx = nn.TransformerDecoder(
            decoder_layer=decoder_layers1,
            num_layers=3
        )
        self.project_codeIdx = nn.Linear(self.model_dim2, self.num_emb)
    ...
    def forward(self, x, x_text):  # Toekn size: T
        ...
        text_feature_tokens = self.pos_encoder(text_feature)

        result_text = self.transformer_decoder(x, text_feature_tokens)
        # result = result_text[1:, :, :] # Tt*NJoints, bs, model_dim

        pre_codes = self.transformer_codeIdx(self.linear_mid_codeIdx(result_text),
                                             self.linear_text_condeIdx(text_feature_tokens))
