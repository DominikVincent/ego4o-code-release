...
import json
import os
from llava.ego4o.constants import MOTION_TO_TEXT_QUESTION_LIST, MOTION_TO_MOTION_QUESTION_LIST, \
    MOTION_ANSWER_LIST, TEXT_TO_MOTION_QUESTION_LIST, MOTION_TEXT_TO_MOTION_QUESTION_LIST
...
def main():
    ...
    json_output_path = os.path.join(nymeria_root_path, 'ego4o_input_json_multi_modal.jsonl')
    ...
    for atomic_file in os.listdir(atomic_path):
        ...
        for i, atomic_data in enumerate(atomic_data_list):
            ...
            motion_description = atomic_data['text']['Describe my atomic actions']
            if len(motion_description) <= 10:
                print(f'Warning: {motion_description} is too short, skipping', flush=True)
                continue
            ...
            data_item['fps'] = 30
            data_item['id'] = new_id
            data_item['motion_id'] = {'human': new_id, }
            ...
            # generate motion to motion data
            data_item = {}
            data_item['image'] = image_name
            data_item['fps'] = 30
            data_item['id'] = new_id
            data_item['motion_id'] = {'human': new_id, 'gpt': new_id}
            data_item['motion_file'] = f'{seq_name}.pkl'
            random_question = np.random.choice(MOTION_TO_MOTION_QUESTION_LIST)
            random_answer = np.random.choice(MOTION_ANSWER_LIST)
            data_item['conversations'] = [
                {
                    'from': 'human',
                    'value': random_question
            ...
            # generate text to motion data
            data_item = {}
            data_item['image'] = image_name
            data_item['fps'] = 30
            data_item['id'] = new_id
            data_item['motion_id'] = {'gpt': new_id}
            data_item['motion_file'] = f'{seq_name}.pkl'
            random_question = np.random.choice(TEXT_TO_MOTION_QUESTION_LIST)
            random_answer = np.random.choice(MOTION_ANSWER_LIST)
            data_item['conversations'] = [
                {
                    'from': 'human',
                    'value': random_question + ' ' + atomic_data['text']['Describe my atomic actions']
                },
                {
                    'from': 'gpt',
                    'value': random_answer
            ...
            # generate motion text to motion data
            data_item = {}
            data_item['image'] = image_name
            data_item['fps'] = 30
            data_item['id'] = new_id
            data_item['motion_id'] = {'human': new_id, 'gpt': new_id}
            data_item['motion_file'] = f'{seq_name}.pkl'
            random_question = np.random.choice(MOTION_TEXT_TO_MOTION_QUESTION_LIST)
            random_answer = np.random.choice(MOTION_ANSWER_LIST)
            data_item['conversations'] = [
                {
                    'from': 'human',
