...
from mmpose.datasets.datasets.imuposer.imuposer_config import amass_combos




@TRANSFORMS.register_module()
class NormalizeIMUMotion(BaseTransform):
    def __init__(self, imu_acc_name, imu_ori_name, imu_acc_mean_std_path, imu_ori_mean_std_path):
        self.imu_acc_name = imu_acc_name
        self.imu_ori_name = imu_ori_name
        self.imu_acc_mean_std_path = imu_acc_mean_std_path
        self.imu_ori_mean_std_path = imu_ori_mean_std_path

        self.imu_acc_mean_std = torch.load(self.imu_acc_mean_std_path)
        self.imu_ori_mean_std = torch.load(self.imu_ori_mean_std_path)
    ...
    def transform(self,
        ...
        # do the normalize
        imu_acc = (imu_acc - self.imu_acc_mean_std['mean']) / self.imu_acc_mean_std['std']
        imu_ori = (imu_ori - self.imu_ori_mean_std['mean']) / self.imu_ori_mean_std['std']
        results[self.imu_acc_name] = imu_acc
        results[self.imu_ori_name] = imu_ori
        return results
    ...
    def inv_normalize(self, results: Dict) -> Optional[Union[Dict, Tuple[List, List]]]:
        ...
        imu_ori = results[self.imu_ori_name]
        # do the unnormalize
        imu_acc = imu_acc * self.imu_acc_mean_std['std'] + self.imu_acc_mean_std['mean']
        imu_ori = imu_ori * self.imu_ori_mean_std['std'] + self.imu_ori_mean_std['mean']
        results[self.imu_acc_name] = imu_acc
        results[self.imu_ori_name] = imu_ori
        return results
...
@TRANSFORMS.register_module()
class NormalizeHMLMotion(BaseTransform):
...
@TRANSFORMS.register_module()
class RotationMatrixTo6D(BaseTransform):
...
@TRANSFORMS.register_module()
class InitAlignIMUMotion(BaseTransform):
    '''
...
@TRANSFORMS.register_module()
class HMLMotionRepresentation(BaseTransform):
    def __init__(self, joint_name, drop_last_pose_name_list=None):
        self.joint_name = joint_name
        self.drop_last_pose_name_list = drop_last_pose_name_list
        from mmpose.utils.humanml_utils.motion_representation import process_file
...
@TRANSFORMS.register_module()
class ChangeHMLShape(BaseTransform):
...
@TRANSFORMS.register_module()
class AddDummyText(BaseTransform):
...
@TRANSFORMS.register_module()
class RandomMaskSensors(BaseTransform):
    def __init__(self, mask_value=0.0, mask_name_list=None, sensor_combo=None):
        self.mask_value = mask_value
        self.mask_name_list = mask_name_list
        self.sensor_combo = sensor_combo

    def transform(self, results: dict) -> dict:
        if self.sensor_combo is None:
            # randomly select a sensor combo
            combo_list = list(amass_combos.keys())
            self.sensor_combo = np.random.choice(combo_list)
        combo_index = amass_combos[self.sensor_combo]
        for name in self.mask_name_list:
            data = results[name]
            result_imu_info = deepcopy(data) * self.mask_value
            result_imu_info[:, combo_index] = data[:, combo_index]
            results[name] = result_imu_info
        return results
