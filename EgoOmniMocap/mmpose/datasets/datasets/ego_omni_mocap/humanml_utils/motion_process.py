...
def recover_global_limb_rot(joint_positions, global_direction=None):
    t2m_joint_number = 22
    device = joint_positions.device
    if global_direction is None:
        # use default global direction, x axis
        global_direction = torch.asarray([1, 0, 0]).float().to(device)[np.newaxis, np.newaxis, ...]
    limb_direction = torch.zeros((len(joint_positions), t2m_joint_number, 3)).float().to(device)  # t2m have 22 joints
    # get the direction of each limb
    ...
    # limb_rot = np.zeros(shape=(len(joint_positions), joints_num, 4))
    # for i in range(1, len(joint_positions)):
    #     limb_rot[i] = qbetween_np(limb_direction[i], global_direction)

    limb_rot = qbetween(limb_direction, global_direction)
    # convert quantion to cont6d
    limb_cont6d = quaternion_to_cont6d(limb_rot)
    limb_cont6d[:, 0] = torch.zeros((len(joint_positions), 6)).float().to(device)
    # print(limb_cont6d)
    return limb_cont6d

def recover_global_limb_rot_batch(joint_positions, global_direction=None):
    t2m_joint_number = 22
    device = joint_positions.device
    if global_direction is None:
        # use default global direction, x axis
        global_direction = torch.asarray([1, 0, 0]).float().to(device)[np.newaxis, np.newaxis, np.newaxis, ...]
    limb_direction = torch.zeros((joint_positions.shape[0], joint_positions.shape[1],
                                  t2m_joint_number, 3)).float().to(device)  # t2m have 22 joints
    # get the direction of each limb
    for chain in t2m_kinematic_chain:
        for i in range(1, len(chain)):
            limb_direction[:, :, chain[i], :] = joint_positions[:, :, chain[i]] - joint_positions[:, :, chain[i-1]]
    ...
    # convert quantion to cont6d
    limb_cont6d = quaternion_to_cont6d(limb_rot)
    limb_cont6d[:, :, 0] = torch.zeros((joint_positions.shape[0], joint_positions.shape[1], 6)).float().to(device)
    # print(limb_cont6d)
    return limb_cont6d
