...
class MeshViewer(object):
    ...
    def add_point_seq(self, point_seq, color=[1.0, 0.0, 0.0], radius=0.015,
        ...
        - connections : array of point index pairs, draws a cylinder between each pair to create skeleton
         <del> - vel : list of Nx3 numpy arrays for the velocities of corresponding sequence points </del>
        - vel : list of Nx2x3 numpy arrays for the velocities / acceleration of corresponding sequence points, the
        first vector is the start point, the second vector is the end point
        '''
        # ensure same length as other sequences
        ...
        if vel is not None:
            ...
            for _, point_vel_pair in enumerate(vel):

                cyl_mesh_list = []
                for point_pair_each in point_vel_pair:
                    p1 = point_pair_each[0]
                    p2 = point_pair_each[1]
                    # breakpoint()

                    segment = np.array([p1, p2])
                    if np.linalg.norm(p1 - p2) < 1e-6:
                        continue
                    cyl_mesh = trimesh.creation.cylinder(radius*0.5, height=None, segment=segment)
                    cyl_mesh.visual.vertex_colors = [0.0, 0.0, 1.0]
                    cyl_mesh_list.append(cyl_mesh.copy())

                if len(cyl_mesh_list) == 0:
                    m = pyrender.Mesh.from_trimesh(trimesh.creation.box((radius, radius, radius)))
                else:
                    # combine
                    m = pyrender.Mesh.from_trimesh(cyl_mesh_list)
    ...
    def animate(self, fps=30, cam_traj=None):
        '''
        ...
        while self.use_offscreen or self.viewer.is_active:
            ...
            if cam_traj is not None:
                camera_pose = self.get_init_cam_pose()
                camera_pose[:3, 3] = camera_pose[:3, 3] + np.array([cam_traj[self.animation_frame_idx, 0], cam_traj[self.animation_frame_idx, 1], 0.0])
