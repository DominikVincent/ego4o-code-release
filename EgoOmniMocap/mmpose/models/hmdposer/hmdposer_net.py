...
class TemporalSpatialBackbone(torch.nn.Module):
    def __init__(self, input_dim, number_layer=3, hidden_size=256, dropout=0.05, nhead=8, block_num=2):
        ...
        self.time_encoder = nn.ModuleList(
            [nn.ModuleList([torch.nn.LSTM(hidden_size, hidden_size, num_rnn_layer,
                                          bidirectional=False, batch_first=True) for _ in range(8)]) for _ in
    ...
    def forward(self, x_in, rnn_state=None):
        ...
        # now x_in is a dict input
        head_feats = [x_in['head_rot'], x_in['head_rot_vel'],
                      x_in['head_acc'], x_in['head_vel']]
        lhand_feats = [x_in['lhand_rot'], x_in['lhand_rot_vel'],
                          x_in['lhand_acc'], x_in['lhand_vel']]
        rhand_feats = [x_in['rhand_rot'], x_in['rhand_rot_vel'],
                            x_in['rhand_acc'], x_in['rhand_vel']]
        lfoot_feats = [x_in['lfoot_rot'], x_in['lfoot_rot_vel'],
                            x_in['lfoot_acc']]
        rfoot_feats = [x_in['rfoot_rot'], x_in['rfoot_rot_vel'],
                            x_in['rfoot_acc']]
        batch_size, time_seq = x_in['head_rot'].shape[0], x_in['head_rot'].shape[1]
        # set pelvis features, lhand_inhead, rhand_inhead to zero
        pelvis_feats = [torch.zeros_like(lfoot_feats[0]), torch.zeros_like(lfoot_feats[1]),
                        torch.zeros_like(lfoot_feats[2])]
        lhand_inhead_feats = [torch.zeros_like(head_feats[0]), torch.zeros_like(head_feats[1]),
                                torch.zeros_like(head_feats[2]), torch.zeros_like(head_feats[3])]
        rhand_inhead_feats = [torch.zeros_like(head_feats[0]), torch.zeros_like(head_feats[1]),
                                torch.zeros_like(head_feats[2]), torch.zeros_like(head_feats[3])]
        ...
        for idx in range(self.num_block):
            ...
            collect_feats = self.spatial_encoder[idx](
                collect_feats_temporal.reshape(batch_size * time_seq, 8, -1)).reshape(batch_size, time_seq, 8, -1)
...
class HMD_imu_HME_Universe(torch.nn.Module):
    ...
    def forward(self, x_in, rnn_state=None):
        ...
        hml_res = self.hml_est(collect_feats)
