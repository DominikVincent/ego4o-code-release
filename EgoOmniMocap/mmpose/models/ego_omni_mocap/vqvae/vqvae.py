...
class VQVAE_limb_hml(nn.Module):
    def __init__(self,
        ...
        self.partSeg = [[0, 1, 2, 3, 4+63+126, 4+63+126+1, 4+63+126+2],  # root
                        [x for i in [3, 6, 9, 12, 15] for x in values_term(i)],  # head
                        [x for i in [13, 16, 18, 20] for x in values_term(i)], # left arm
                        [x for i in [14, 17, 19, 21] for x in values_term(i)],  # right arm
                        [x for i in [1, 4, 7, 10] for x in values_term(i)] + [259, 260],  # left leg
                        [x for i in [2, 5, 8, 11] for x in values_term(i)] + [261, 262]]  # right leg
...
class HumanVQVAE(BaseModel):
    def __init__(self,
        ...
        if args is None:
            from .parser_util import mtm_args
            args = mtm_args()
        self.mtm_args = args
        self.nb_joints = 22
    ...
    def loss(self, motion_hml):
        x_out, commit_loss, perplexity = self.forward_feature(motion_hml)
        loss_dict = {}
        recon_loss = F.mse_loss(x_out, motion_hml)
        loss_dict['recon_loss'] = recon_loss
        # loss_dict['commit_loss'] = commit_loss * self.mtm_args.commit
        loss_dict['commit_loss'] = commit_loss
        return loss_dict

    def predict(self, motion_hml):
        x_out, loss, perplexity = self.forward_feature(motion_hml)
        return x_out

    def forward(self, motion_hml, data_samples = None, mode: str = 'tensor', **kwargs):
        if mode == 'tensor':
            return self.forward_feature(motion_hml)
        elif mode == 'predict':
            predictions = self.predict(motion_hml)
            return predictions
        elif mode == 'loss':
            loss = self.loss(motion_hml)
            return loss
