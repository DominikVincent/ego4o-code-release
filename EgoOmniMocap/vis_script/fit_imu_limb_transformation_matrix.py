...
def visualize_hml3d(raw_result_path, out_dir):
    ...
    for i, result in enumerate(results):
        ...
        body_joints_all.extend(body_joints)
        imu_ori_all.extend(imu_ori)

    body_joints_all = np.array(body_joints_all)
    imu_ori_all = np.array(imu_ori_all)
    ...
    imu_ori_mat_y_axis = imu_ori_all[:, imu_poser_id, 1, :]
    ...
    # right_wrist_ori = body_joints_all[:, smpl_id, :] - body_joints_all[:, smpl_id - 2, :]
    ...
    # left_leg_ori = body_joints_all[:, smpl_id, :] - body_joints_all[:, smpl_id - 3, :]
    ...
    right_leg_ori = body_joints_all[:, smpl_id, :] - body_joints_all[:, smpl_id - 3, :]
    right_leg_ori = right_leg_ori / np.linalg.norm(right_leg_ori)
    rot_mat = find_optimal_rotation_matrix(imu_ori_mat_y_axis, right_leg_ori)
